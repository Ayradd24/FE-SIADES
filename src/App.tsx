...
import KatalogJasa from './pages/admin/KatalogJasa';
import AdminLayout from './components/layout/AdminLayout';
import AdminRoutes from './router/AdminRoutes';

const previewJasa = [
  { nama: 'Bambang', jasa: 'Servis Listrik/Elektronik', deskripsi: 'Layanan servis listrik dan elektronik terpercaya...', harga: 'Rp 30.000', satuan: '/Barang', color: 'bg-blue-100', icon: '🔧' },
  { nama: 'Warni', jasa: 'Salon & Kecantikan', deskripsi: 'Potong rambut, creambath, smoothing, dan perawatan wajah...', harga: 'Rp 15.000', color: 'bg-pink-100', icon: '💇' },
  { nama: 'Fauzi', jasa: 'Fotografer', deskripsi: 'Jasa foto pernikahan, wisuda, produk, dan dokumentasi acara...', harga: 'Rp 300.000', satuan: '/acara', color: 'bg-violet-100', icon: '📷' },
  { nama: 'Dian', jasa: 'Laundry', deskripsi: 'Cuci setrika ekspres, satuan, dan kiloan. Tersedia layanan jemput...', harga: 'Rp 6.000', satuan: '/Kg', color: 'bg-teal-100', icon: '🧺' },
  { nama: 'Heru', jasa: 'Tukang Bangunan', deskripsi: 'Jasa renovasi rumah, pasang keramik, dan pengecatan dinding...', harga: 'Rp 150.000', satuan: '/hari', color: 'bg-orange-100', icon: '🏗️' },
];
    ...
    <div className="font-sans">
      ...
      >
        ...
        <div className="relative text-center px-4 max-w-2xl">
          ...
          <button
            onClick={() => document.getElementById('katalog-jasa')?.scrollIntoView({ behavior: 'smooth' })}
      ...
      <div id="katalog-jasa" className="py-12 bg-[#e8edf5]">
        <h2 className="text-xl font-bold text-blue-800 mb-8 text-center">KATALOG JASA WARGA</h2>
        <div className="max-w-6xl mx-auto px-4">
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
            {previewJasa.map((item, i) => (
              <div key={i} className="bg-white rounded-2xl shadow-sm overflow-hidden hover:shadow-md transition-shadow duration-200 flex flex-col">
                <div className={`${item.color} h-28 flex items-center justify-center`}>
                  <span className="text-4xl">{item.icon}</span>
                </div>
                <div className="p-3 flex flex-col flex-1">
                  <h3 className="font-bold text-blue-800 text-xs">{item.nama}</h3>
                  <p className="font-semibold text-gray-800 text-xs">{item.jasa}</p>
                  <p className="text-xs text-gray-500 mt-1 flex-1 line-clamp-2">{item.deskripsi}</p>
                  <div className="mt-2">
                    <span className="font-bold text-gray-800 text-xs">{item.harga}</span>
