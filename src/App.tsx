  ...
  useEffect(() => {
    api
      ...
      .then((res) => {
        ...
        const top = all
          .filter((o) =>
            ["kepala desa", "sekretaris desa", "bendahara desa"].includes(
              o.jabatan.toLowerCase(),
            ),
          )
          .sort(
            (a, b) =>
              (order[a.jabatan.toLowerCase()] ?? 99) -
              (order[b.jabatan.toLowerCase()] ?? 99),
          )
          .slice(0, 3);
        setItems(top);
      })
      ...
      <div className="mx-auto grid max-w-4xl grid-cols-1 justify-items-center gap-6 sm:grid-cols-3">
        {loading ? (
          Array.from({ length: 3 }).map((_, i) => (
            <div
              key={i}
              className="bg-white p-5 rounded-xl w-52 shadow animate-pulse"
            >
          ...
          items.map((item) => (
            <div
              key={item.id}
              className={`bg-white p-5 rounded-xl w-52 shadow hover:shadow-md transition-shadow ${getPreviewColumnClass(item.jabatan)}`}
            >
              <div className="w-16 h-16 bg-blue-200 mx-auto rounded-full mb-2 overflow-hidden flex items-center justify-center">
                {item.foto ? (
                  <img
                    src={`${storageBase}${item.foto}`}
                    alt={item.nama}
                    className="w-full h-full object-cover"
                  />
                ) : (
                  <svg
                    className="w-8 h-8 text-blue-800/60"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
